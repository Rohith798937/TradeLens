...
GROQ_API_KEY = "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"
def generate_response(prompt):
    ...
    try:
        ...
        data = response.json()
        return data.get("choices", [{}])[0].get("message", {}).get("content", "No response from TLens AI.")
    except Exception as e:
        return f"Error: {e}"

# TradeLens dashboard
def TradeLens():
    st.title("TradeLens - Stock Dashboard")
    ...
    # Tabs
    tabs = st.tabs(["Pricing Data", "Fundamental Data", "News", "Sentiment Indicator", "TLens Chatbot"])
